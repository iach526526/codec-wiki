...
Opus 在 [opus-codec.org](https://opus-codec.org/) 上被描述為一款「完全開放、免版稅、高度多功能的音頻編碼器。Opus 在互動式語音和音樂傳輸方面無可比擬，但也適用於儲存和串流應用。它由網際網路工程任務組（IETF）標準化為 [RFC 6716](https://datatracker.ietf.org/doc/html/rfc6716)，該標準融合了 Skype 的 SILK 編碼器和 Xiph.Org 的 CELT 編碼器技術。」
...
- 支持固定位元率（constant bitrate,CBR）和可變位元率（variable bitrate,VBR）
- 音頻帶寬範圍：從窄頻到全頻
- 支持語音和音樂
- 支持單聲道和立體聲
- 支持最多 255 通道（multistream frames）
- 動態調整位元率、音頻帶寬和框大小
...
- `--bitrate #.###` 設定整體目標位元率，單位為 kbit/s。大多數編碼器使用 *比特* 每秒，所以你需要指定「128K」來表示 128 kbit/s。例如，Opus 不遵循這一點，所以你只需輸入「128」，但請注意，使用高效的 VBR 編碼可能最終位元率會不同於目標。Opus 支持的位元率範圍為 6 kb/s 至 510 kb/s。

- `--vbr` 告訴編碼器使用可變位元率，根據需要分配更多或更少的比特以保留每比特的整體音質。這是本地儲存和播放的最佳選擇，並且 *默認啟用*。
...
- `--comp #` 設定編碼器複雜度的值，範圍從 0 到 10，0 為最簡單，10 為最複雜。*默認為 10*。

- `--framesize #` 設定最大編碼器框大小，單位為毫秒。降低此值有助於改善延遲，但會犧牲每比特的音質。值得注意的是，40ms 和 60ms 框大小只是多個 20ms 框的組合，通過 opusenc 的默認行為處理，它們不被認為有用，因為它們會降低編碼器的適應性，可能會使延遲和編碼效率變差。*默認值為 20ms*。

- `--expect-loss #` 預期的封包丟失百分比。對於本地編碼和播放不太有用，但對於實時應用有用。*默認值為 0*。

- `--downmix-mono` 將多通道音頻降混為單聲道。

- `--downmix-stereo` 將多通道音頻降混為兩個通道，即左聲道和右聲道，前提是提供給編碼器的通道數超過兩個。

- `--no-phase-inv` 禁用相位反轉。當將立體聲降混為單聲道時有幫助，雖然在這種情況下這是默認行為，自 libopus 1.3 起。略微降低立體聲音質。

- `--max-delay #` 設定最大容器延遲，單位為毫秒，範圍為 0-1000。*默認為 1000ms*。

查看編碼器標誌的默認值，opusenc 幾乎總是遵循最佳實踐進行設定。這使得使用非常簡單，只需插入某種來源並使用最基本的命令即可進行 Opus 編碼。
---
