...
<Tabs>
    ...
    <TabItem value="dec" label="解碼">
        ...
        預設情況下，如果 `.jxl` 檔案是通過無損 jpeg 重新壓縮的，`djxl` 將還原原始的 jpeg 檔案。若想避免這樣做，並生成一個新的 jpeg 檔案：
        ```bash
...
[libjxl-tiny](https://github.com/libjxl/libjxl-tiny) 包含了一個簡化版的 JPEG XL 編碼器實現，專為不包含 alpha 通道的攝影圖像設計。它旨在指導硬件實現編碼器，其中無法支援全套編碼工具。顏色管理不在此庫的範疇內，編碼器輸入作為線性 sRGB 色彩空間中的便攜浮點映射（PFM）提供，其中單個樣本值可以超出 \[0.0, 1.0\] 範圍以處理超出色域的顏色。更多細節，請參見 [編碼工具概述](https://github.com/libjxl/libjxl-tiny/blob/main/doc/coding_tools.md)。

最近的提交是在十個月前，因此不確定 libjxl-tiny 是否可以被視為活躍的。

### Hydrium

[Hydrium](https://github.com/Traneptora/hydrium) 是一個快速、超低內存、流式 JPEG XL 編碼器，用便攜 C 編寫。由 Traneptora 維護。

### zune-jpegxl

[zune-jpegxl](https://github.com/etemesi254/zune-image/tree/dev/crates/zune-jpegxl) 是一個簡單、快速且完全安全的模塊化 JXL 編碼器，使用 Rust 編寫。由 etemesi254 維護。
...
[jxl-oxide](https://github.com/tirr-c/jxl-oxide) 是一個符合規範的 JPEG XL 解碼器，使用純 Rust 編寫。由 Wonwoo Choi 維護。

---

來源：

- [JXL 維基百科](https://en.wikipedia.org/wiki/JPEG_XL)
- [JPEGXL.info: 為什麼選擇 JXL](https://jpegxl.info/why-jxl.html)
- [Apple JXL 宣布](https://webkit.org/blog/14205/news-from-wwdc23-webkit-features-in-safari-17-beta/#images)
- [JPEG XL: 起源與現狀](https://cloudinary.com/blog/jpeg-xl-how-it-started-how-its-going)
- [支援 JPEG XL 的理由](https://cloudinary.com/blog/the-case-for-jpeg-xl)
- [下一代編解碼器取代 JPEG 的時機](https://cloudinary.com/blog/time_for_next_gen_codecs_to_dethrone_jpeg)
- [圖像編解碼器比較](https://giannirosato.com/blog/post/image-comparison/)