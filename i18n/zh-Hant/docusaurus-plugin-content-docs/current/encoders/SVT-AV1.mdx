...
SVT-AV1 在 x86 架構的 CPU 上表現最佳，而在 ARM 架構上則表現不佳。雖然從 1.8.0 版本開始，ARM NEON 組合指令的支援有所改善，但 SVT-AV1 在 ARM 上的性能仍然有限，因此不適合作為跨架構 CPU 的性能基準。此外，SVT-AV1 的 AV1 功能支援也有限，它只支援最高 4:2:0 色度子取樣，且不支援 12 位顏色，也不支援場景變化檢測（未來也無計劃實施此功能）。SVT-AV1 編碼器生成的最小影片尺寸為 64x64。
...
啟用量化矩陣，默認情況下禁用。這可以提高編碼速度和效率，生成的影片品質基本保持不變。

- `--qm-min 0`

將量化矩陣的最小平滑度設定為 0，默認值為 8。除非處理極重的顆粒，否則建議使用此設定。量化矩陣的最大平滑度為 15，應保持默認值不變。
...
SVT-AV1-PSY 對主線 SVT-AV1 編碼器進行了多項功能增強，並且修改了默認設定，使得更容易產生感知上更優化的位源流。關於該編碼器的完整功能增強列表和默認值修改內容，請參閱[專案的 README](https://github.com/gianni-rosato/svt-av1-psy/blob/master/README.md#feature-additions)。