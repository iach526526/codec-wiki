...
SVT-AV1-PSY 在 [Aviator](../utilities/Aviator.mdx) 中作為默認編碼器使用，也可以在 [rAV1ator CLI](../utilities/rav1ator-cli.mdx) 中使用，通過該工具提供的預編譯二進位檔或自己編譯一個二進位檔來使用。
...
提供對我們增強的 AQ 模式 0 和 2 的控制，這些模式可以利用每幀的變異訊息，以在高/低對比度場景下實現更一致的品質。提供了四個曲線選項，默認為曲線 2。1：輕微，2：溫和，3：中等，4：激進。

- `--variance-octile` *1 至 8* (**[合併至主線](https://gitlab.com/AOMediaCodec/SVT-AV1/-/merge_requests/2195)**)

控制算法在提升超塊時的「選擇性」，基於其低/高 8x8 變異比率。值為 1 是最不選擇性的，如果超塊的 1/8 變異低則會輕易提升。相反，值為 8 則只有在整個超塊的變異都低時才會提升。較低的值會增加位元率。默認值為 6。

- `--enable-alt-curve` *0 和 1*

啟用替代的變異提升曲線，具有不同的比特分配和視覺特徵。默認為 0。
...
用於修改環路濾鏡去塊的銳利度和比特失真率，以改善視覺保真度的參數。默認為 0（無銳利度）。
