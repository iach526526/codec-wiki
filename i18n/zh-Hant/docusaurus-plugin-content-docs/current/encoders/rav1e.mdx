...
:::
        ...
        rav1e 目前有一個 [尚未合併的拉取請求](https://github.com/xiph/rav1e/pull/3000) 由 quietvoid 提交，這位開發者也負責 [hdr10plus_tool](../utilities/hdr10plus_tool.mdx) 和 [dovi_tool](../utilities/dovi_tool.mdx)。該 PR 添加了一個名為 `--hdr10plus-json` 的新參數，用於 HDR10+ JSON 動態元資料輸入。要在本地合併此修補程序，請執行以下步驟：
