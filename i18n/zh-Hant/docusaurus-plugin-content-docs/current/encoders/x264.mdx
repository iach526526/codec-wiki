...
x264 是一個用於編碼 H.264 / AVC 的軟體庫和命令行應用程式，由 VideoLAN 開發。VideoLAN 也開發了廣受歡迎的 VLC 媒體播放器，x264 採用 GNU GPL 授權釋出。該庫使用 C 和 Assembly 語言編寫，擁有近二十年的開發歷史和線程優化，這使它成為最快的軟體影片編碼器之一，並且極受歡迎。
...
默認設定為可用核心數量的 1.5 倍，除非你需要減少執行緒數量，否則不需要調整。x264 的執行緒處理非常高效，通常 x264 可以完全利用大多數消費級 CPU。
