...
通過 avifenc 使用 aomenc 來編碼 AVIF 圖像，通常被視為最佳選擇，因為 SVT-AV1 僅支援 4:2:0 色度抽樣，而 rav1e 的編碼速度對於靜態圖像來說不夠快，且 libaom 團隊在內部編碼方面比其他主要的開源 AV1 編碼器團隊投入了更多精力。下面是一個 AVIF 編碼的範例命令：
...
- `-y 444` 是色度抽樣模式。4:4:4 色度抽樣通常提供更好的壓縮效果，但在某些圖像上，4:2:0 色度抽樣可能是更好的選擇。
...
- `--arnr-strength=1` 控制過濾（平滑）的強度，這一直是個熱門話題。大多數人建議使用默認的 4，一些人認為 1 適合 3D 像 Pixar CGI 和 2D 動畫，而 4 適合真人內容；對於低位元率編碼則使用更高的值。

- `--disable-kf --enable-fwd-kf=0` 我們禁用了關鍵幀，因為 Av1an 已經進行了場景檢測，這樣可以加快編碼速度。

- `--kf-max-dist=9999` 設定最大關鍵幀間隔為最大值，因為 Av1an 的場景檢測默認的關鍵幀間隔為 240。
...
遺憾的是，主線版本的 aomenc 並不完美。它存在錯誤的默認設定、過度依賴感知上有缺陷的 [PSNR](../metrics/PSNR.mdx) 指標、誤導性設定及其他問題。幸運的是，程式社群開發了幾個分支來解決 aomenc 的根本問題。
