...
[Kvazaar](https://github.com/ultravideo/kvazaar) 是一個開源的 [H.265 / HEVC](/docs/video/HEVC) 軟件編碼器，由 [Ultra Video Group](https://ultravideo.fi/) 開發，使用 C 語言編寫，並遵循 BSD 3 條款許可證。
...
```bash title="簡單的 Y4M 輸入，QP 20，原始 265 比特流輸出"
...
以下命令仍使用 `kvazaar` 二進制檔，但從 YUV4MPEG 管道而不是文件中讀取。這對於將 FFmpeg 的輸出管道傳遞給 Kvazaar 很有用。
